        ...
        for label in self.label:
            ...
            for i in range(len(case)):
                ...
                if flag:
                    ...
                    prob = prob
                    # if prior:
                    #     prob = prob * float(prior[label])
                    # else:
                    #     prob = prob * self.prior[label]
            posterior.append((prob, label))
            norm += prob
        #print posterior, norm, "hi"
        #return max(posterior)[0] / norm, max(posterior)[1] # on when making predictions
        return posterior # on when generating graphs
